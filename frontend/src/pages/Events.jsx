...
import { fetchEvents } from "../api"; // Centralized API uplink

/**
 * @description Mission Catalog (Events Page)
 * Hardened with Archival Protocol and a 5-event historical buffer.
 */
  ...
  useEffect(() => {
    const loadAllEvents = async () => {
      try {
        ...
        /**
         * @section Auto-Categorization & Archival Protocol
         * 1. Filter out all Archived missions (isArchived: true).
         * 2. Sort by date descending (Newest first).
         */
        const visibleMissions = allData
          .filter(e => !e.isArchived) // RESTORED: Hide archived data
          .sort((a, b) => new Date(b.date) - new Date(a.date));

        // Upcoming Missions Logic
        const upcoming = visibleMissions.filter(e => {
          const eventDate = new Date(e.date);
          return e.status === "upcoming" && eventDate >= now;
        });

        // Historical Archives Logic (Limited to 5 recent past events)
        const history = visibleMissions
          .filter(e => {
            const eventDate = new Date(e.date);
            return e.status !== "upcoming" || eventDate < now;
          })
          .slice(0, 5); // RESTORED: Limit of 5 recent past events
      ...
      <div className="relative z-10 px-6 pt-32 pb-24 max-w-7xl mx-auto mt-12">
        ...
        <div className="text-center mb-24">
          ...
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.85] mb-8 uppercase italic">
            Upcoming <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FFD700] to-yellow-500">Events</span>
          </h1>
          ...
          {loading ? (
            ...
            <AnimatePresence>
              {upcomingEvents.map((event) => (
                <motion.div key={event._id} layout initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="h-full">
                  <EventCard
                    id={event._id}
                    title={event.title}
                    date={event.date}
        ...
        <motion.div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3 mb-24" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          {loading ? (
             Array(3).fill(0).map((_, i) => <Skeleton key={i} variant="card" />)
          ) : pastEvents.length > 0 ? (
            pastEvents.map((event) => (
              <motion.div key={event._id} className="h-full">
                <EventCard
                  id={event._id}
                  title={event.title}
                  date={event.date}
