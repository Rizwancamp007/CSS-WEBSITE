 ...
 * Hardened for secure data collection and transmission to the Command Center.
 * Features industrial glassmorphism and real-time validation feedback.
 */
export default function Membership() {
   ...
   * Broadcasts the applicant's credentials to the Board Registry.
   */
  const handleSubmit = async (e) => {
    e.preventDefault();
    
    // Transmission Validation: Payload Integrity Check
    ...
    try {
       ...
       * Hits the hardened /memberships endpoint established in Phase 21.
       */
      ...
      if (response.ok && result.success) {
        ...
        setError(result.message || "UPLINK_DENIED: Record with this Node ID already exists.");
      }
    } catch (error) {
      setError("COMMUNICATION_FAILURE: Mainframe link unstable.");
    } finally {
            ...
            {submitted && (
              ...
              >
                ...
                <p className="text-[11px] font-black text-slate-500 max-w-sm uppercase tracking-widest leading-relaxed">
                  The Executive Board is reviewing your credentials. Check your digital mail for confirmation and future activation protocols.
                </p>
              </motion.div>
            )}
        ...
        {!submitted && (
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="IDENTIFIER" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">University Roll No</label>
              <input type="text" name="rollNo" value={formData.rollNo} onChange={handleChange} placeholder="NODE_ID" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">Department Sector</label>
              <input type="text" name="department" value={formData.department} onChange={handleChange} placeholder="SECTOR" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">Enrollment Phase</label>
              <input type="text" name="semester" value={formData.semester} onChange={handleChange} placeholder="PHASE_INDEX" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">Primary Uplink (Gmail)</label>
              <input type="email" name="gmail" value={formData.gmail} onChange={handleChange} placeholder="COMM_LINK" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>

            <div className="space-y-1">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">Signal Line (WhatsApp)</label>
              <input type="text" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} placeholder="CONTACT_ID" className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-900 focus:border-blue-500/50 transition-all outline-none font-bold text-sm" required />
            </div>
            ...
            <div className="md:col-span-2 space-y-1">
              ...
              <div className="relative">
                <select 
                    ...
                    className="w-full px-6 py-5 rounded-2xl bg-slate-950/80 border border-slate-800 text-white focus:border-blue-500/50 transition-all outline-none appearance-none cursor-pointer text-[10px] font-black uppercase tracking-[0.2em]"
                    required
                >
            ...
            <div className="md:col-span-2 pt-8">
              <button
                ...
                className={`w-full py-6 rounded-2xl bg-white text-black font-black uppercase tracking-[0.3em] text-[11px] shadow-3xl hover:bg-[#FFD700] transition-all duration-500 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed`}
              >
