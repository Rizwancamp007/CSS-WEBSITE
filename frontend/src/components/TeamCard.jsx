 ...
 * Hardened to reflect society hierarchy with robust image fallback and 
 * secure social uplinks.
 */
export default function TeamCard({ 
  ...
  image, 
  description, 
  socials = {} // Refined: Expects the socials object from the backend
}) {
  
  /**
   * @section Asset Handshake
   * Logic: Priority to database image -> Localized Fallback -> Remote Fallback
   */
  const imageSrc = image && image.trim() !== "" 
    ? image 
    : "/assets/images/member-placeholder.jpg"; 
          ...
          <div className="relative w-36 h-36 rounded-full overflow-hidden border-4 border-slate-800 group-hover:border-[#FFD700]/50 transition-all duration-500 shadow-2xl bg-slate-950">
            <img
              ...
              onError={(e) => {
                ...
                e.currentTarget.src = "https://placehold.co/400x400/020617/FFD700?text=Node+Inactive";
              }}
            />
          </div>
        </div>
